{
  ...
  **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
  ...
  let sesliKanalID = client.channels.cache.get("929720579664269313");
  if (sesliKanalID)
    sesliKanalID.join()
...
client.on('message', async (msg, member) => {
    ...
    if (msg.content === 'general kimdir'){
          
        msg.reply('titanın adam gibi adam abisi');
    }
  
  
  
    if (msg.content === 'yusuf ab kimdir') {
      
        msg.reply('Adamın dibi Yusuf abdır tersini diyen şerefsizdir ama şiyar ab hariç');
    }
  
  
  
    if (msg.content === 'tmm ab kızma affet') {
      
        msg.reply('tmm la kaybol gözümden.Bu arada ORDER AB KRALDIR bunuda unutma') ;
    }
  
  
  
    if (msg.content === 'halil kimdir') {
      
        msg.reply('ŦłŦ卂几 ⱠƗØ₦un en kral adamın dibi sınıf arkadaşıdır')
    }
  
    if (msg.content === 'owo bot nedir') {
      
        msg.reply('kumar etkisi yaratan ve cimrinin teki olan oç bir bottur')
    }
  
  
  
    if (msg.content === 'öyle bir oyun yok lan mal') {
      
        msg.reply('boş yapma lan sensin mal bizi kodlayan kişi titan lionsa öyle bir oyun vardır');
    }
  
  
  
    if (msg.content === 'ne yapıyorsun') {
      
        msg.reply('Titan Ab Kraldır oynuyorum');
    }
  
  
  
    if (msg.content === 'ee bot nasılsın') {
      
        msg.reply('iyi seni sormalı');
    }
  
  
  
    if (msg.content === 'naber bot') {
      
        msg.reply('iyi ne olsun senden?');
    }
  
  
  
    if (msg.content === '🜍 Taha kimdir') {
      
        msg.reply('31 manyağı olan ve ŦłŦ卂几 ⱠƗØ₦un sınıf arkadaşı olan biri');
    }
  
  
  
    if (msg.content === 'order kimdir?') {
      
        msg.reply('titan liona 150k owo atan adamın dibi bir abimiz');
    }
  
  
  
    if (msg.content === 'only woux kimdir?') {
      
        msg.reply('adamın dibi ali abidir');
    }
  
  
  
    if (msg.content === 'goddar kimdir?') {
      
       msg.reply('titan liona yazılım öğretecek olan adamın dibi bir abimiz')
    }
  
  
  
    if (msg.content === 'seni kim kodladı') {
        
        msg.reply('@ŦłŦ卂几 ⱠƗØ₦#4691');
    }   
    ...
    if (msg.content === 'hi bot') {
        ...
        msg.reply('türkçe konuş la anlamıyom');
    }
   
    if (msg.content === 'yunus kim') {
       
        msg.reply('kudurtan ve napimi bulan kişidir.');
    }
    
    if (msg.content === 'söyle bakalım bot şiyar ab kimdir') {
       
        msg.reply('30fps pubg kralı ve pubg manyağıdır diyara burdan selamlar diyar noobsun ağlake');
    }              
